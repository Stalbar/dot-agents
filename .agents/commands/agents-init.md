...
2. Main language(s) and versions.
3. Frameworks (web, ORM, CLI, data).
4. Package manager and core commands.
5. Test framework and the exact command that runs the full suite.
6. Lint/format tools and commands.
7. Databases/storage and any split (config vs analytics).
8. Infrastructure (containers, cloud, CI) and the files that define it.
9. Where environment variables are declared (all files that must stay in sync).
10. API framework and docs tool, if any.
11. Notebooks or scripts that must be importable, if any.
12. Documentation language (recommend: English).
13. External services and how credentials are provided.
14. Anything else that makes rules project-specific.

## Step 4 - Fill and generate

- Fill `architecture.md` (all 11 mandatory sections), `project_overview.md`,
  `coding_standards.md`, `credentials.md` from the templates with my answers.
...
- Verify the root bootstrap files exist (`AGENTS.md`, `GEMINI.md`, `CLAUDE.md`,
  `.cursor/rules/agents.mdc`). Create any that are missing.
